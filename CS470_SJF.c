...
void srtf(Process proc[]) {
    ...
    while (completed != n) {
        ...
        //Process will execute when index is not -1
        if (index != -1) { 
            proc[index].remaining_time--;
            
            // when executiong is started, print execution order
            if (prev != index) {
                printf("P%d ", proc[index].process_id);
                prev = index;
            }
            
            // once remaining time is 0, process has been completed
            if (proc[index].remaining_time == 0) {
                proc[index].is_completed = 1;
                completed++;
                
                // Calculating waiting time
                proc[index].waiting_time = current_time + 1 - proc[index].arrival_time - proc[index].burst_time;
                
                // Calculating turnaround time
                proc[index].turnaround_time = proc[index].waiting_time + proc[index].burst_time;
            }
        }
        current_time++;
    }
